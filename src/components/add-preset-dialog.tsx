...
export function AddPresetDialog({ open, onOpenChange, onPresetAdded }: AddPresetDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const newPreset = await addPreset(name, color, isPrivate, password);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Presety są domyślnie publiczne. Przełącz, aby uczynić go prywatnym i ustawić hasło.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="is-private-preset" className="text-right">Prywatny</Label>
              <div className="col-span-3 flex items-center gap-2">
                 <Switch id="is-private-preset" checked={isPrivate} onCheckedChange={setIsPrivate} />

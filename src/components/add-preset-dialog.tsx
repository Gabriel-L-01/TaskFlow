...
interface AddPresetDialogProps {
  ...
  onPresetAdded: (preset: Preset) => void;
  userId: string | null | undefined;
}

export function AddPresetDialog({ open, onOpenChange, onPresetAdded, userId }: AddPresetDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
     ...
     if (isPrivate && !password) {
        toast({ variant: 'destructive', title: 'Hasło jest wymagane dla prywatnych presetów' });
        return;
    }

    setSubmitting(true);
    try {
      const newPreset = await addPreset(name, color, privacyLevel, password, userId);
      onPresetAdded(newPreset);
      toast({ title: 'Preset utworzony!', description: `"${name}" został dodany.` });
      onOpenChange(false);
      // Reset form
      setName('');
      ...
      toast({ variant: 'destructive', title: 'Błąd', description: `Nie udało się utworzyć presetu: ${error.message}` });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handlePrivacyChange = () => {
    ...
    if (newLevel === 'personal' && !userId) {
        toast({ variant: 'destructive', title: 'Funkcja niedostępna', description: 'Musisz być zalogowany, aby tworzyć presety osobiste.'});
        return;
    }
  ...
  const privacyConfig = {
    public: { icon: Globe, label: 'Publiczna', description: 'Każdy może zobaczyć ten preset.' },
    private: { icon: Lock, label: 'Prywatny', description: 'Wymagane hasło do dostępu.' },
    personal: { icon: UserIcon, label: 'Osobisty', description: 'Tylko Ty możesz zobaczyć ten preset.' },
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Dodaj nowy preset</DialogTitle>
          <DialogDescription>
             Ustaw nazwę, kolor i poziom prywatności dla swojego nowego presetu.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            {isPrivate && (
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="password-preset" className="text-right">Hasło</Label>
                <PasswordInput
                  id="password-preset"
                  value={password}
          ...
          <DialogFooter>
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? 'Dodawanie...' : 'Dodaj preset'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>

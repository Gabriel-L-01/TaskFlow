...
export function AddNoteDialog({ open, onOpenChange, onNoteAdded, userId }: AddNoteDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isPrivate && !password) {
        toast({ variant: 'destructive', title: 'Hasło jest wymagane dla notatek prywatnych' });
        return;
    }

    setSubmitting(true);
    try {
      const content = `# ${name.trim()}\n\nZacznij pisać swoją notatkę tutaj...`;
      const newNote = await addNote(name.trim(), color, privacyLevel, content, password, userId);
      onNoteAdded(newNote);
      toast({ title: 'Notatka utworzona!', description: `"${name}" została dodana.` });
      onOpenChange(false);
      // Reset form
      setName('');
      ...
      setPassword('');
        ...
        toast({ variant: 'destructive', title: 'Błąd', description: `Nie udało się utworzyć notatki: ${error.message}` });
    } finally {
        setSubmitting(false);
    }
  };
  ...
  const handlePrivacyChange = () => {
    ...
    if (newLevel === 'personal' && !userId) {
        toast({ variant: 'destructive', title: 'Funkcja niedostępna', description: 'Musisz być zalogowany, aby tworzyć notatki osobiste.'});
        return;
    }
  ...
  const privacyConfig = {
    ...
    personal: { icon: UserIcon, label: 'Osobista', description: 'Tylko Ty możesz zobaczyć tę notatkę.' },
  };
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Dodaj nową notatkę</DialogTitle>
           <DialogDescription>
            Ustaw nazwę, kolor i poziom prywatności dla swojej nowej notatki.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="name-note" className="text-right">Tytuł</Label>
              <Input
                id="name-note"
                value={name}
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="color-note" className="text-right">Kolor</Label>
